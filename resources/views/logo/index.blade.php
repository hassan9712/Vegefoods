        ...
        <tbody> 
            ...
            @foreach($logos as $logo)
                <tr role="row" class="odd">
                    <td><img src="{{ asset('images/' . $logo->image) }}" style="width:250px; height:150px"></td>
                    <td style="text-align:center">
                        <form action="{{ route('dashboard.logos.destroy', $logo->id) }}" style="display:inline" method="POST">
                            @method('DELETE')
                            @csrf
                            <button class="btn btn-danger waves-effect waves-light"> <i class="fa fa fa-trash-o"></i> </button>
                        </form>
                        <a href="dashboard/logos/{{ $logo->id }}/edit">
